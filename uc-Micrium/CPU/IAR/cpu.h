...
#if 0
关键部分配置
*注意：（1）使用CPU /编译器的临界区方法配置CPU_CFG_CRITICAL_METHOD：
*通过...输入/退出关键部分
* CPU_CRITICAL_METHOD_INT_DIS_EN禁用/启用中断
* CPU_CRITICAL_METHOD_STATUS_STK将中断状态推送/
弹出到堆栈上CPU_CRITICAL_METHOD_STATUS_LOCAL将中断状态保存/恢复到本地变量
*
（a）CPU_CRITICAL_METHOD_INT_DIS_EN不是首选方法，因为它不支持多个级别的中断。但是，对于
某些/编译器，这是唯一可用的方法。
*
（b）CPU_CRITICAL_METHOD_STATUS_STK是一种首选方法，因为它支持多个级别的中断。但是，此方法
假定编译器为以下各项提供C级和/或程序集级功能：
*输入关键部分：
*（1）将中断状态推送/保存到本地堆栈
*（2）禁用中断
*退出关键部分：
*（3）从本地堆栈弹出/恢复中断状态
*
（c）CPU_CRITICAL_METHOD_STATUS_LOCAL是一种首选方法，因为它支持多个级别的中断。但是，此方法
假定编译器为以下各项提供C级和/或程序集级功能：
*输入关键部分：
*（1）将中断状态保存到本地变量中
*（2）禁用中断
...
*（3）从局部变量恢复中断状态
*
（2）关键部分宏很可能需要内联汇编。如果编译器不允许在C源文件中进行内联汇编，则关
键节宏必须调用在以下软件目录中的'cpu_a.asm'文件中定义的汇编子例程：
* \ <CPU编译器目录> \ <cpu> \ <编译器> \ where
* <CPU编译器目录>通用CPU编译器软件的目录路径
* <cpu>特定CPU的目录名称
* <compiler>特定编译器的目录名称
*（3）（a）为了保存/恢复中断状态，可能需要声明类型为'CPU_SR'的本地变量'cpu_sr'
（例如，如果配置了'CPU_CRITICAL_METHOD_STATUS_LOCAL'方法）。
*（1）应该通过CPU_SR_ALLOC（）宏声明“ 
cpu_sr”局部变量，如果使用该宏，则必须在所有其他局部变量之后声明。
*示例：
* void Fnct（void）
* {
* CPU_INT08U val_08;
...
* CPU_INT32U val_32;
注意:
* CPU_SR_ALLOC（）;   必须在所有其他局部变量之后声明
*：
*：
*}
*（b）将“ CPU_SR”数据类型配置为适当大小的CPU数据类型，其大小应足以完全存储CPU /
编译器的状态字。

#endif

#ifdef  FREERTOS_CRITICAL

#include "FreeRTOS.h"
#define  CPU_CFG_CRITICAL_METHOD    CPU_CRITICAL_METHOD_STATUS_LOCAL

#define  CPU_SR_ALLOC()
#define  CPU_CRITICAL_ENTER()         portENTER_CRITICAL()
#define  CPU_CRITICAL_EXIT()          portEXIT_CRITICAL() 


#else
#error  "CPU_CRITICAL_ENTER() and CPU_CRITICAL_EXIT() define error"
